...
INIT_FUNC(mod_alias_checked_init) {
	plugin_data *p;
...
FREE_FUNC(mod_alias_checked_free) {
	plugin_data *p = p_d;
...
SETDEFAULTS_FUNC(mod_alias_checked_set_defaults) {
	plugin_data *p = p_d;
	size_t i = 0;
...
#define PATCH(x) \
	p->conf.x = s->x;
static int mod_alias_checked_patch_connection(server *srv, connection *con, plugin_data *p) {
	size_t i, j;
...
PHYSICALPATH_FUNC(mod_alias_checked_physical_handler) {
	plugin_data *p = p_d;
	...
	mod_alias_checked_patch_connection(srv, con, p);
...
int mod_alias_checked_plugin_init(plugin *p);
int mod_alias_checked_plugin_init(plugin *p) {
	p->version     = LIGHTTPD_VERSION_ID;
	p->name        = buffer_init_string("alias_checked");

	p->init           = mod_alias_checked_init;
	p->handle_physical= mod_alias_checked_physical_handler;
	p->set_defaults   = mod_alias_checked_set_defaults;
	p->cleanup        = mod_alias_checked_free;
